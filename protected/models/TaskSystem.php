...
class TaskSystem extends CActiveRecord
{
	...
	public function attributeLabels()
	{
		return array(
			...
			'modified_by' => 'Последний редактировал',
			'create_time' => 'Задание создано',
			'modify_time' => 'Последние изменения',
	...
	public function TaskFileExists($id)
	{
		$result = Yii::app()->db->createCommand("SELECT task_file FROM `o_task_system` WHERE id = ".$id."")->queryAll();
 		$res1 = $result[0]['task_file'];
 		return $res1;
	}

	public function ExecutorFileExists($id)
	{
		$result = Yii::app()->db->createCommand("SELECT executor_file FROM `o_task_system` WHERE id = ".$id."")->queryAll();
